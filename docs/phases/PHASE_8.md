...
> **Superseded (post-launch nav simplification):** `/history/insights` was later merged into `/history` itself — one page now shows the open case (if any), the dashboard, and the closed-case list together. The nav label for this page changed from "Verdicts" to "The Archive." See the `nav-simplification-and-landing` change for details.
